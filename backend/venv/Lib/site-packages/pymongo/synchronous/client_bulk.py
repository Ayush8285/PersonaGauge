...
class _ClientBulk:
    ...
    def _process_results_cursor(
        ...
        if result.get("cursor"):
            coll = Collection(
                database=Database(self.client, "admin"), name="$cmd.bulkWrite",
            )
    ...
    def execute_command(
        ...
        def retryable_bulk(
            ...
            self._execute_command(
                self.write_concern, session, conn, op_id, retryable, full_result,
            )
    ...
    def execute_command_unack(self, conn: Connection,) -> None:
    ...
    def execute_no_results(self, conn: Connection,) -> None:
    ...
    def execute(self, session: Optional[ClientSession], operation: str,) -> Any:
        """Execute operations."""
        if not self.ops:
        ...
        return ClientBulkWriteResult(
            result, self.write_concern.acknowledged, self.verbose_results,
        )
